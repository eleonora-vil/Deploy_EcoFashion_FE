...
const MaterialsAll: React.FC = () => {
 const { data: materials = [], isLoading, error } = useQuery<MaterialDetailDto[]>({
   ...
   queryFn: async () => {
     try {
       const result = await materialService.getAllMaterialsAdmin();
       console.log('✅ Materials loaded:', result.length);
       return result;
     } catch (err) {
 ...
 React.useEffect(() => {
   if (error) {
     console.error('MaterialsAll error:', error);
   }
 }, [error]);
           ...
           <div className="card-body">
             {isLoading ? (
               ...
               <div className="text-center py-8">
                 ...
                 <div className="text-sm text-gray-500">{(error as Error).message || 'Unknown error'}</div>
               </div>
             ) : materials.length === 0 ? (
