...
export default function OrdersDetails() {
  ...
  const { user, supplierProfile, designerProfile, loadUserProfile } = useAuthStore(); // Get current user info and profiles
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Check if user is supplier and handle accordingly
        if (user?.role === 'supplier') {
          // Ensure supplier profile is loaded
          let currentSupplierProfile = supplierProfile;
          if (!currentSupplierProfile) {
            console.log('Supplier profile not loaded, attempting to load...');
            await loadUserProfile();
            // Get updated supplier profile after loading
            const { supplierProfile: updatedSupplierProfile } = useAuthStore.getState();
            currentSupplierProfile = updatedSupplierProfile;
            if (!currentSupplierProfile?.supplierId) {
              throw new Error('Unable to load supplier profile. Please refresh the page.');
            }
          }
          ...
          const supplierOrders = await ordersService.getOrdersBySeller(supplierId);
          const specificOrder = supplierOrders.find(order => order.orderId === Number(orderId));

          if (specificOrder) {
            setData(specificOrder);
            // Get order details using supplier-specific method
            const lines = await ordersService.getOrderDetailsBySupplier(Number(orderId), supplierId);
            setDetails(lines);
          } else {
            throw new Error('Order not found or you do not have permission to view this order');
          }
        } else if (user?.role === 'designer') {
          // For designers, get designerId from designerProfile loaded by authStore
          let currentDesignerProfile = designerProfile;
          if (!currentDesignerProfile) {
            console.log('Designer profile not loaded, attempting to load...');
            await loadUserProfile();
            // Get updated designer profile after loading
            const { designerProfile: updatedDesignerProfile } = useAuthStore.getState();
            currentDesignerProfile = updatedDesignerProfile;
            if (!currentDesignerProfile?.designerId) {
              throw new Error('Unable to load designer profile. Please refresh the page.');
            }
          }
          ...
          if (specificOrder) {
            setData(specificOrder);
            // Get order details using designer-specific method
            const lines = await ordersService.getOrderDetailsByDesigner(Number(orderId), designerId);
            setDetails(lines);
          } else {
            throw new Error('Order not found or you do not have permission to view this order');
          }
        } else {
          // For customers and other roles, use original logic
